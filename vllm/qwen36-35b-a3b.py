...
def build_prompt_token_ids(tokenizer, target_len):
    ...
    return [random.randint(0, vocab_size - 1) for _ in range(target_len)]
def main():
    random.seed(0)
    cc.resume();
    inst = itt.domain_create("inst.vLLM.Qwen3.6-35B-A3B");
    print("\033[41mStart Init collection\033[0m")
    itt.task_begin(inst, "Initialization")
    llm = LLM(model="/home/chaos/prjs/models/Qwen3.6-35B-A3B",revision=None, 
    ...
    itt.task_end(inst)
    print("\033[41mStop Init collection\033[0m")
    ...
    for in_len in INPUT_LENS:
        ...
        print(f"\033[36mInput length target={in_len}, actual={actual_in_len}\033[0m")
