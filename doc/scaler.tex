...
ext_modules.append(  ####添加一个SyclExtension, 必须放在首个ext_modules = [...]之后
    SyclExtension(
        name="custom_esimd_kernels_vllm.custom_esimd_kernels_newKernelName",
        sources=[
            "csrc/xpu/esimd_kernel_newKernelName.sycl",
            "csrc/xpu/torch_extension_newKernelName.cc",
        ],
        include_dirs=[
            root / "include",
            root / "csrc",
...
\end{lstlisting}
	...
	\begin{enumerate}
	    \item 创建新算子头文件
\begin{lstlisting}[language=c++]
//csrc/xpu/esimd_kernels/newKernelName.h
...
template<....>
struct newKernelName{
    ....
    void operator()(sycl::nd_item<..> it) const SYCL_ESIMD_KERNEL{ ##kernel的operator()必须加SYCL_ESIMD_KERNEL否则不会按照ESIMD编译. 同样如果会被kernel调用的esimd函数要加上SYCL_ESIMD_FUNCTION来修饰
	...
    }
};

template<...>
inline void newKernelName_host(...){
    q.submit([&](sycl::handle& h){
	h.parallel_for(sycl::nd_range....,
	newKernelName<tempalteParam1, templateParam2...>{...,...,...});
    });
}
\end{lstlisting}
	    \item sycl文件中注册
...
#include "esimd_kernels/newKernelName.h" //
at::Tensor esimd_newKernelName(at::Tensor input, .....){
    ...
    auto& q = get_device_queue(input);
    EXTRACT_PTR(p_in, input);

    newKernelName_host<...>(p_in, ...);
}
\end{lstlisting}
	    \item 注册到torch dispatcher
\begin{lstlisting}[language=c++]
TORCH_LIBRARY(custom_esimd_kernels_vllm, m){
    ...
    m.def("esimd_newKernelName(ParamType paramName, ...) - > Tensor");
    m.impl("esimd_newKernelName", torch::kXPU, &esimd_newKernelName);
    ...
}
\end{lstlisting}
	    \item 注册python wrapper
\begin{lstlisting}[language=python]
##python/custom_esimd_kernels_vllm/ops.py
def esimd_newKernelName(...) -> torch.Tensor:
    ...
    return _ops.esimd_newKernelName(....);
...
\end{lstlisting}
    \item 测试
\begin{lstlisting}[language=python]
#scalerDev/ws/sBase/vllm/custom-esimd-kernels-vllm/tests/test_newKernelName.py
import sys, torch
